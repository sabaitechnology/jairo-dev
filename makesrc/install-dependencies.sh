...
### This script probably needs some conditional handling for whether we're doing
### a $development install or not.
...
### Dumb way of checking if there are any further upgrades, but it works.
### Could be foiled by updates requiring input if user is unsure what to do.
# while [ $(apt-get -s upgrade | grep -c Inst) -gt 0 ]; do
